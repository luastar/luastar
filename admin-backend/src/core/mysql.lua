...
function _M:get_connect()
  ...
  if not db then
    logger.error("failed to instantiate mysql : ", err)
    return nil
  end
  -- 设置超时时间
  ...
  if not ok then
    logger.error("failed to connect mysql : err = ", err, ", errcode = ", errcode, ", sqlstate = ", sqlstate)
    db:close()
    return nil
  end
  return db
...
function _M:multi_query_transaction(sql_array)
  ...
  if not res_st then
    logger.error("start transaction failed : err = ", err_st, ", errcode = : ", errcode_st, ", sqlstate = ", sqlstate_st)
    self:close(connect)
  ...
  for i, sql in ipairs(sql_array) do
    ...
    if not res then
      ...
      if not res_rb then
        logger.error("rollback failed : err = ", err_rb, ", errcode = ", errcode_rb, ", sqlstate = ", sqlstate_rb)
      end
      self:close(connect)
      return result
    end
  ...
  if not res_commit then
    logger.error("commit failed : err = ", err_commit, ", errcode = ", errcode_commit, ", sqlstate = ", sqlstate_commit)
  end
  self:close(connect)
  return result
end
...
function _M:close(connect)
  ...
  if not ok then
    logger.error("set keepalive failed : ", err)
  end
end
