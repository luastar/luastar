...
function _M:new(config_file)
	logger.info("[Bean:init] file : ", config_file)
	if not config_file then
		logger.error("[Bean:init] illegal argument : config_file can't nil.")
		return
	end
	local instance = {
...
function _M:create_bean(id, ctime)
	...
	if ctime[id] == bean_status.init_ing then
		...
		ctime[id] = nil
		logger.error("[Bean:create_bean] id ", id, " has circular dependency.")
		return bean
	end
	ctime[id] = bean_status.init_ing
	local bean_config = self.config[id] or {}
	logger.debug("[Bean:create_bean] ", id, " config : " .. cjson.encode(bean_config))
	if not bean_config.class then
		bean.status = bean_status.init_fail
		ctime[id] = nil
		logger.error("[Bean:create_bean] ", id, " config class is null.")
		return bean
	end
	...
	if not ok then
		bean.status = bean_status.init_fail
		ctime[id] = nil
		logger.error("[Bean:create_bean] ", id, "  require class fail.")
...
function _M:get_value(key)
	...
	if not var then
		logger.debug("[Bean:get_value] key[", key, "] value : ", key)
		return key
	end
	...
	if var_len == 1 then
		logger.debug("[Bean:get_value] key[", key, "] value : ", cjson.encode(val))
		return val
	end
	...
	logger.debug("[Bean:get_value] key[", key, "] value : ", cjson.encode(rs))
	return rs
end
