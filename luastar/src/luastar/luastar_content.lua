...
function init()
    ...
    execute_var["ctrl_config"] = execute_var["route"]:getRoute(ngx.var.uri, ngx.var.request_method)
    ...
    -- 获取拦截器
    execute_var["interceptorAry"] = execute_var["route"]:getInterceptor(ngx.var.uri, ngx.var.request_method)
end
...
function execute_ctrl_new()
    local interceptor_ok, interceptor_msg, interceptor_code = execute_before()
...
function execute_before()
    ...
    for key, value in pairs(execute_var["interceptorAry"]) do
        ...
        if call_ok and _.isFunction(interceptor["beforeHandle"]) then
            call_ok, rs_ok, rs_msg, rs_code = pcall(interceptor["beforeHandle"])
            if call_ok then
                -- 有一个返回失败，则返回
                if not rs_ok then
